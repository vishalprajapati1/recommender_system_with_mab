...
class NCF(torch.nn.Module):
    def __init__(self, num_users, num_items, factors=8, layers=[64, 32, 16, 8]):
        ...
        # GMF part
        self.embedding_user_gmf = torch.nn.Embedding(num_users, factors)
        self.embedding_item_gmf = torch.nn.Embedding(num_items, factors)

        # MLP part
        self.embedding_user_mlp = torch.nn.Embedding(num_users, layers[0] // 2)
        self.embedding_item_mlp = torch.nn.Embedding(num_items, layers[0] // 2)

        # MLP layers
        self.fc_layers = torch.nn.ModuleList()
        for idx in range(1, len(layers)):
            self.fc_layers.append(torch.nn.Linear(layers[idx - 1], layers[idx]))

        # Final prediction layer
        self.affine_output = torch.nn.Linear(layers[-1] + factors, 1)
